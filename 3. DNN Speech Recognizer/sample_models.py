...
from keras.layers import (BatchNormalization, Conv1D, Dense, Input, Dropout, 
    TimeDistributed, Activation, Bidirectional, SimpleRNN, GRU, LSTM, ZeroPadding1D)
...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for i in range(recur_layers):
        simp_rnn = GRU(units, activation='relu', return_sequences=True, implementation=2, name='rnn_{}'.format(i))(input_data if i == 0 else bn_rnn)
        bn_rnn = BatchNormalization()(simp_rnn)
    # DONE: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(bn_rnn)
    # Add softmax activation layer
...
def final_model(input_dim, filters, kernel_size, conv_stride,
    ...
    bn_rnn =  BatchNormalization()(input_data)  
    conv_1d = Conv1D(filters, kernel_size, 
                     strides=conv_stride, 
                     padding=conv_border_mode,
                     activation='relu',
    ...
    for i in range(recur_layers):
        bidir_rnn = Bidirectional(GRU(units, 
                                      ...
                                      implementation=2,
                                      dropout=0.5,
                                      name='rnn_{}'.format(i)), merge_mode='sum')(bn_rnn if i == 0 else bidir_rnn)
    bn_rnn = BatchNormalization()(bidir_rnn)     
    time_dense = TimeDistributed(Dense(filters, activation='relu'))(bn_rnn)    
    # DONE: Add softmax activation layer
    y_pred = TimeDistributed(Dense(output_dim, name="y_pred", activation="softmax"))(time_dense)
    # Specify the model
    model = Model(inputs=input_data, outputs=y_pred)
    # DONE: Specify model.output_length
    model.output_length = model.output_length = lambda x: cnn_output_length(x, kernel_size, conv_border_mode, conv_stride)
    print(model.summary())
    return model